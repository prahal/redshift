...
gnomerr_start(gnomerr_state_t *state)
{
	...
	for (int i = 0; crtcs[i] != NULL && i < state->crtc_count; i++) {
		...
		state->crtcs[i].is_gamma_valid = TRUE;
		...
		if (ramp_size == 0)
			state->crtcs[i].is_gamma_valid = FALSE;
...
gnomerr_restore(gnomerr_state_t *state)
{
	...
	for (int i = 0; i < state->crtc_count; i++) {
		if (state->crtcs[i].is_gamma_valid == FALSE)
			return;
...
gnomerr_print_help(FILE *f)
{
	...
	fputs(_("  crtc=N\tList of comma separated CRTCs to apply"
		" adjustments to\n"),
	      f);
	fputs("\n", f);
}
...
static int
gnomerr_set_temperature_for_crtc(
	 gnomerr_state_t *state, int crtc_num, const color_setting_t *setting,
	 int preserve)
{
	 GError *error;
	...
	if (state->crtcs[crtc_num].is_gamma_valid == FALSE)
		return 0;
...
static int
gnomerr_set_temperature(
	 gnomerr_state_t *state,const color_setting_t *setting, int preserve)
{
	int r;
